...
- **Full-sky** (`fullsky=True`): Wigner 3j coupling from closed-form lookup tables (Kiddier & Gratton 2026, `augr.wigner_closed`; issue #48), one fused kernel per L instead of a Schulten-Gordon recursion — per-L table 2.3× (spin-2) to 12× (m=0) faster than the recursion it replaced — and the five N_0 estimators evaluated on a sampled L grid (`n_L_sample="auto"`: every L < 20 plus ~100-130 log-spaced points, `N_0^{-1}` log-interpolated; `None` restores the exact every-L sweep). The sampled grid was set by a value-and-derivative convergence study (`scripts/n0_validation/l_sampling_convergence.py`): at the default, `A_lens_eff` and its noise derivative are within 5e-4 of the dense grid on PICO-like and LiteBIRD-like noise at `l_max_qe` 1000-1500, for ~13× fewer Wigner sweeps per estimator at 1500. Run `scripts/bench_wigner_closed.py` on an idle machine for the before/after wall-clock table. TT, EE, EB, TB validated against `plancklens` to <1e-3 in bulk-L (10..2000) at the LiteBIRD-PTEP fiducial; TE to <3e-2 in (10, 1800) (2.35e-2 measured; the remaining residual is the single-projection OkaHu Table I form vs plancklens's symmetric `pte+pet`, see `scripts/n0_validation/derivation.md`; <0.1% effect on `N_0^MV` and <1% on `A_L`). **Production-grade for space-mission applications** (where the reionization bump `l ≲ 10` dominates the σ(r) constraint and the `(L+1)²/L²` flat-vs-full geometric correction matters at low L), and **the default inside the design forward** (`delens_residual_bb`, `DelensCoupling.build`, `make_optimization_context(delens=...)`; pass `fullsky=False` / `delens_fullsky=False` for flat-sky). Besides the geometry, it is the parallel path: the flat-sky QE's scan over l1 is serial by construction, and on a 144-core node its design gradient at `l_max_qe=1500` ran 175 s against full-sky sampled's 14 s. **Set `JAX_NUM_CPU_DEVICES=N` in the environment before Python starts** to cut the per-L sweep into `N` concurrent slices (JAX reads the variable once, at import; `_map` then splits the L grid automatically, and `AUGR_DELENS_NO_SHARD=1` opts out). `N` counts JAX logical CPU devices — a software partition of the one physical CPU into `N` shard targets, not `N` cores and not GPUs; all `N` share the node's XLA thread pool. Measured on Vista gg, 144 cores (job 974631), values agreeing to 1.7e-16: the full-sky design gradient at `l_max_qe=3000` runs 52.1 s at 1 slice (8.0 GB), 16.9 s at 4 (25.3 GB), 14.8 s at 16 (93.8 GB); at `l_max_qe=1500`, 14.1 s → 4.1 s from 1 slice to 4. **Use `N=4` on a 144-core node**: 16 buys 12% more wall time for 3.7× the memory. The sampled L grid is 117 / 125 / 142 / 149 points at `l_max_qe` 800 / 1500 / 3000 / 4000, so 4 slices carry ~30 L each; scale `N` down on a smaller node, or under an outer process pool, which must divide it the way it already divides `AUGR_DELENS_WORKERS`. `iterate_delensing` does not benefit (7.0 / 7.1 / 8.5 s at 1 / 4 / 16 slices — five estimators in sequence that already fan out).
...
For nested-pool callers (outer pool calling into `iterate_delensing` per worker), `process_pool` automatically sets `AUGR_DELENS_WORKERS = max(1, cpu_count // n_workers)` for the children so total CPU use ≈ `cpu_count`. Override with `process_pool(n, delens_workers=K)`. The same arithmetic applies to `JAX_NUM_CPU_DEVICES` if you shard the full-sky per-L map (above): it is not managed for you, because JAX fixes the device count at import and a child cannot change it after the fact — set it per worker before the process starts, or leave it at 1.
