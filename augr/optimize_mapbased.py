...
Phase 1a scope -- the noise lever. The differentiable knobs are the per-band
detector count ``n_det``, NET ``net``, total efficiency ``eta_total``, and the
scalar ``mission_years`` -- everything that enters the white-noise power ``w_inv``
via :func:`augr.instrument.white_noise_power_continuous`. Beams are held concrete
(the cleaner's needlet-channel mask is discrete in beams; see the plan's
beam-path constraint), so aperture / band-center optimization stays on the
analytic path. The chain is:
